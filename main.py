...
Deploy on Cloud Run. Connects to Supabase (PostgreSQL).

Cloud Run fixes applied:
  - Listens on 0.0.0.0 (not 127.0.0.1)
  - Reads port from $PORT env var (default 8080)
  - Startup done via __main__ block with uvicorn
"""
...
# Set these in GCP Cloud Run → Edit & Deploy → Variables & Secrets
...
@app.patch("/assets/{asset_id}", summary="Update asset [auth required]")
def update_asset(
    asset_id: int,
    payload: AssetPatch,
    _user: dict = Depends(get_current_user),
...
# ── Entrypoint — Cloud Run reads $PORT, binds to 0.0.0.0 ─────────────────────
if __name__ == "__main__":
    uvicorn.run(
        "main:app",
        host="0.0.0.0",   # required by Cloud Run
        port=PORT,         # reads $PORT env var
        reload=False,
    )
