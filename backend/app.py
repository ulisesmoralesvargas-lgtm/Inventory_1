...
Calls the FastAPI backend deployed on Railway.
"""
...
API_URL: str = st.secrets.get("RAILWAY_API_URL", os.getenv("RAILWAY_API_URL", "https://inventory-1-production.up.railway.app"))
...
def ref_map(table: str) -> dict[str, int]:
    """Returns {name: id} mapping for a reference table."""
    return {row["name"]: row["id"] for row in fetch_ref(table)}

def id_to_name(table: str) -> dict[int, str]:
    return {row["id"]: row["name"] for row in fetch_ref(table)}
...
def page_update():
    ...
    # Optionally load current values
    ...
    with st.form("update_form"):
        ...
        new_notes     = st.text_area("Notes (leave blank to keep)", value="")
